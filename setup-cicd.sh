  ...
  deploy:
    ...
    - name: 🧪 Test Deployment
      run: |
        ...
        if curl -f -s https://${SERVER_HOST}/health | grep -q "healthy"; then
          echo "✅ Health check passed!"
        else
          echo "❌ Health check failed!"
          curl -s https://${SERVER_HOST}/health || echo "Failed to reach health endpoint"
          exit 1
        fi
        
        # Test WiFi API endpoint
        echo "🔍 Testing WiFi API endpoint..."
        if curl -f -s https://${SERVER_HOST}/api/wifi | grep -q "ssid"; then
          echo "✅ WiFi API is working!"
        else
          echo "❌ WiFi API failed!"
          curl -s https://${SERVER_HOST}/api/wifi || echo "Failed to reach WiFi API"
          exit 1
        fi
